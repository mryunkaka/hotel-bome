...
<body>
    ...
    <table class="items">
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="7" class="center">No rooms</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <table class="two-col kv-narrow">
        <tr>
            <td class="left" style="width:70%">
                <div><span class="k">{{ $billTo['input'] ?? 'Company Name' }}</span><span class="v">:
                        {{ $companyName ?? '-' }}</span></div>
                <div><span class="k">Address</span><span class="v">: {{ $billTo['address'] ?? '-' }}</span>
                </div>
                <div><span class="k">City</span><span class="v">:
                        {{ $billTo['city'] ?? ($hotel?->city ?? '-') }}</span></div>
                <div><span class="k">Phone</span><span class="v">: {{ $billTo['phone'] ?? '-' }}</span></div>
                <div><span class="k">Handphone</span><span class="v">:
                        {{ $billTo['mobile'] ?? ($billTo['phone'] ?? '-') }}</span></div>
            ...
            <td class="left" style="width:30%">
                ...
                <div><span class="k">Clerk/Booked By</span><span class="v">: {{ $clerkName ?? '-' }}</span>
                </div>
                <div><span class="k">Entry Date</span><span class="v">:
                        {{ \App\Support\ReservationView::fmtDate($issuedAt ?? ($generatedAt ?? now()), false) }}</span>
                </div>

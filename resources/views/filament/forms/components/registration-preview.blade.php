...
@else
    <style>
        .reg-card { border:1px solid #e5e7eb; border-radius:.75rem; padding:1rem; background:#fff; }
        .reg-grid { display:grid; grid-template-columns: 1.3fr 1fr; gap:1rem; }
        .reg-head { display:flex; justify-content:space-between; align-items:center; font-weight:600; margin-bottom:.75rem; }
        .reg-row  { display:grid; grid-template-columns: 160px 1fr; gap:.5rem; padding:.5rem .6rem; border-bottom:1px solid #e5e7eb; }
        .reg-row:first-of-type { border-top:1px solid #e5e7eb; }
        .box { border:1px solid #e5e7eb; border-radius:.5rem; overflow:hidden; background:#fafafa; }
        .box > .titlebar { padding:.6rem .75rem; font-weight:700; background:#f3f4f6; border-bottom:1px solid #e5e7eb; }
        .muted { color:#6b7280; font-size:.9rem; }
        .big   { font-size:1.05rem; font-weight:700; }
        .money { font-variant-numeric: tabular-nums; }
        .btns  { display:flex; gap:.5rem; flex-wrap:wrap; }
        .tag   { display:inline-block; padding:.15rem .6rem; border-radius:999px; background:#eef2ff; color:#3730a3; font-size:.75rem; border:1px solid #c7d2fe; }
        .sep   { height:1px; background:#e5e7eb; margin:.5rem 0; }

        /* Tabel ringkasan angka (kiri bawah) */
        .kv { width:100%; border-collapse:collapse; margin-top:.25rem; background:#fff; }
        .kv tr{ border-bottom:1px solid #e5e7eb; }
        .kv tr:first-child{ border-top:1px solid #e5e7eb; }
        .kv td{ padding:.5rem .6rem; vertical-align:top; }
        .kv td.k{ width:55%; color:#6b7280; }
        .kv td.v{ width:45%; text-align:right; font-variant-numeric: tabular-nums; }

        .ttl   { display:flex; justify-content:space-between; align-items:center; gap:1rem; padding:.6rem .75rem; background:#fff; border-top:2px solid #d1d5db; border-bottom:1px solid #e5e7eb; }
        .ttl .label{ color:#374151; font-weight:700; }
        .ttl .val{ font-weight:800; font-size:1.05rem; }

        /* Kanan juga pakai garis pada setiap row */
        .box .group { background:#fff; }
        .box .group .reg-row { background:#fff; }
    </style>
        ...
        <div class="reg-grid">
            ...
            <div class="box">
                ...
                <div class="group">
                    ...
                    <div class="reg-row"><div>Guest Type</div><div>{{ $guest?->guest_type ?? '-' }}</div></div>
                    ...
                    <div class="reg-row">
                        <div>Birth</div>
                        <div>{{ $guest?->birth_place ? $guest->birth_place.', ' : '' }} {{ $guest?->birth_date?->translatedFormat('d F Y') ?? '-' }}</div>
                    </div>
                    <div class="reg-row">
                        <div>Issued</div>
                        <div>{{ $guest?->issued_place ? $guest->issued_place.', ' : '' }} {{ $guest?->issued_date?->translatedFormat('d F Y') ?? '-' }}</div>
                    </div>
            ...
            <div class="box">
                ...
                <div class="group">
                    ...
                    <div class="reg-row"><div>Breakfast</div><div>{{ $rg?->breakfast ?? '-' }}</div></div>
                    <div class="reg-row"><div>Note</div><div>{{ $rg?->note ?? '-' }}</div></div>
                </div>
            </div>
        </div>
    </div>

// bridge-auto.js — HID/Serial + Keyboard-Wedge Mirroring (DLock-friendly)
...
const LARAVEL_API = (process.env.LARAVEL_API || 'https://hotel-bome.test').replace(/\/+$/, '');
...
const WEDGE_MIN_LEN = Number(process.env.WEDGE_MIN_LEN || 6);    // minimal panjang UID
const WEDGE_MAX_LEN = Number(process.env.WEDGE_MAX_LEN || 24);   // maksimal (8/14/20 umum)
const WEDGE_IDLE_TIMEOUT_MS = Number(process.env.WEDGE_IDLE_TIMEOUT_MS || 250); // flush jika idle
...
async function fetchWithTimeout(url, opts = {}, timeoutMs = 5000) {
    ...
    try {
        ...
        const res = await fetch(url, merged);   // <- PAKAI url yang dikirim dari pemanggil
        return res;
    } finally {
        clearTimeout(id);
    }
        ...
        while (attempt <= RETRIES && !sent) {
            ...
            try {
                ...
                const res = await fetchWithTimeout(url, {
                    ...
                    body: JSON.stringify({ uid: nextUid, scanned_at: new Date().toISOString(), reader_id: wedgeActive ? 'bridge-wedge' : 'bridge-hid' }),
                }, FETCH_TIMEOUT_MS);
                ...
                const text = summarizeText(await res.text().catch(() => ''), 500);
                log('ERROR', `❌ HTTP ${res.status} dari Laravel`, { body: text });

                if ([403, 404, 419].includes(res.status)) break; // perbaikan sisi Laravel diperlukan
            } catch (e) {
...
async function autoDetectDevice() {
    ...
    const skip = d => {
        ...
        if (d.vendorId === 0x04b8) return true;
        return false;
    };
    ...
    const known = all.filter(d =>
        !skip(d) && (
            (d.vendorId === 0x5458 && d.productId === 0x0002) ||
...
function startWedge() {
    ...
    const isHexKey = (k) => {
        const c = String.fromCharCode(k).toUpperCase();
        return /[0-9A-F]/.test(c);
    };

    iohook.on('keypress', e => {
        // e.keychar = ASCII code (0 jika non-printable)
        const kc = e.keychar || 0;

        // terminator
        if (WEDGE_TERMINATOR === 'enter' && e.keycode === 28) { // Enter
            flush('enter');
            return;
        }
        if (WEDGE_TERMINATOR === 'tab' && e.keycode === 15) { // Tab
            flush('tab');
            return;
        }
...
(async function main() {
    ...
    // WEDGE policy:
    // - jika BRIDGE_WEDGE=1 → pakai wedge langsung, abaikan HID
    // - jika BRIDGE_WEDGE=auto → jika HID gagal (kemungkinan dipegang DLock) → aktifkan wedge
    // - jika BRIDGE_WEDGE=0 → tidak pernah wedge
